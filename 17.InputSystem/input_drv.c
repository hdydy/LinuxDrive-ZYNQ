...
static void key_work(struct work_struct *work)
{
    ...
    int value;                          //储存按键状态
    value = gpio_get_value(MIO_PIN_42); //读取按键值
...
static __init int input_drv_init(void)
{
    ...
    */
    ...
    //在key_work工作队列中添加一个key_work任务
    INIT_WORK(&input_dev.key_work, key_work);

    printk("-input_drv_init finish.-\n");
    return 0;
}
...
static __exit void input_drv_exit(void)
{
    ...
    */
    ...
    //释放input_dev结构体变量的内存空间
    input_free_device(input_dev.inputdev);

    printk("-input_drv_exit finish.-\n");
}
